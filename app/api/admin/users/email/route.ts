...
export async function POST(request: Request) {
  try {
    ...
    if (!to || !subject || !html || !userId) {
      return NextResponse.json({ error: 'To, subject, html body, and user ID are required' }, { status: 400 })
    }
    ...
    if (logErr) {
      console.error('[Admin Email API] Log error:', logErr)
    }

    return NextResponse.json({ success: true })
  } catch (err: any) {
    console.error('[Admin User Email API Error]', err)
